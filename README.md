...
### Software
- File-based storage system for authorized RFID card data
- Access control logic with authentication validation
...
2. **Verify**: The system reads the card's unique identifier and checks it against stored data in file storage.

3. **Respond**: The script processes the authentication and sends a response:
   - `GRANTED` - Card is authorized
   - `DENIED` - Card is not authorized

4. **Actuate**: Based on the response:
   - **If GRANTED**:
     - Green LED activates to indicate successful authentication
     - 5V relay triggers to open the door lock
     - Door remains unlocked for 5 seconds before automatically relocking
   - **If DENIED**:
     - Red LED activates to indicate failed authentication
...
- **Visual Feedback**: LED indicators provide immediate visual confirmation of access status
...
- **File-Based Storage**: Simple and portable data storage for authorized cards
